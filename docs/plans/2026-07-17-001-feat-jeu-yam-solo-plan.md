...
- R6. Colonne Tam : jouable uniquement sur annonce faite immédiatement après le 1er lancer du tour ; l'annonce désigne une case précise, encore vide, de la colonne Tam ; le joueur conserve ses lancers restants ; en fin de tour, l'inscription dans la case annoncée est obligatoire, réussie ou non (ratée → 0) ; l'ordre dans la colonne est libre ; annoncer reste facultatif à chaque tour. Quand plus aucune relance n'est possible (3ᵉ lancer atteint), cette inscription obligatoire se fait automatiquement, sans clic ni confirmation — le score réalisé (ou 0) s'inscrit d'office dans la case annoncée (session-settled : user-directed 2026-07-18) ; s'arrêter avant le 3ᵉ lancer reste un choix manuel via la case. L'enchaînement est cadencé pour rester lisible (session-settled : user-directed 2026-07-18, « on n'a pas le temps de voir les dés du 3ᵉ lancer ») : ~1,4 s pour voir le dernier lancer, inscription, ~1,1 s pour la voir, puis le lancer suivant ; en mode À distance, le lancer automatique à l'arrivée du trait attend aussi ~1,2 s après l'inscription adverse. Agir soi-même pendant ces temps de lecture prend simplement la main. À l'écran, l'annonce se fait en touchant une case vide de la colonne Tam juste après le 1ᵉʳ lancer, avec une confirmation dédiée (« Annoncer Tam sur cette case ? »), distincte de la confirmation d'inscription de fin de tour.
...
- R8. Toute case peut recevoir 0 point — contrat manqué ou sacrifice par inscription à sa valeur naturelle (le barrage volontaire d'une case qui rapporte n'a plus d'affordance à l'écran, voir R19) — dans le respect des contraintes de sa colonne : l'ordre pour Descendante et Montante, l'annonce préalable (ratée) pour Tam, l'absence de relance ce tour-ci pour la Sèche. L'appli interdit toute action qui laisserait le tour sans coup légal : si seules des cases Sèche restent jouables, la relance est bloquée avec explication ; si seules des cases Tam restent, l'annonce est proposée d'office après le 1ᵉʳ lancer.
...
- R19. Toucher une case affiche le détail des points qu'elle rapporterait avec les dés actuels et demande confirmation avant d'inscrire ; les cases jouables avec les dés du moment sont signalées visuellement. Toucher une autre case remplace l'aperçu en cours ; toucher hors de la grille (ou « Annuler ») ferme l'aperçu sans rien inscrire. L'aperçu propose une seule action d'inscription : « Inscrire » quand la case rapporte des points, « Inscrire 0 » quand elle vaudrait 0 (contrat manqué ou contrainte violée). Il n'existe pas de barrage volontaire d'une case qui rapporte (session-settled : user-directed 2026-07-18, après les premières parties multi — « cette option ne sert à rien dans aucun cas » ; le sacrifice d'une case se fait en l'inscrivant à sa valeur naturelle, 0 y compris ; le moteur conserve ses gardes de barrage strictes pour Tam et Sèche, testées AE4/AE5). Le premier lancer d'un tour qui suit une inscription part automatiquement, sans clic (tous modes : aussitôt en solo, après la validation du passage de téléphone en À plusieurs, à l'arrivée du trait en À distance) — session-settled : user-directed 2026-07-18 ; le tout premier lancer d'une partie, d'une manche rejouée ou d'une reprise de sauvegarde reste manuel.
